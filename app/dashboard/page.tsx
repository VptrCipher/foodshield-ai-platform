...
export default function DashboardPage() {
  const [auditStatus, setAuditStatus] = useState('INITIATE SITE AUDIT');
  const [isAuditing, setIsAuditing] = useState(false);

  const handleAudit = () => {
    if (isAuditing) return;
    setIsAuditing(true);
    setAuditStatus('DISPATCHING RESOURCES...');
    setTimeout(() => {
      setAuditStatus('AUDIT DISPATCHED 🛡️');
      setTimeout(() => {
        setIsAuditing(false);
        setAuditStatus('INITIATE SITE AUDIT');
      }, 3000);
    }, 2000);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            >
              ...
              <motion.button
                onClick={handleAudit}
                whileHover={!isAuditing ? { scale: 1.02 } : {}}
                whileTap={!isAuditing ? { scale: 0.98 } : {}}
                className={`w-full mt-8 py-3.5 font-bold rounded-xl flex items-center justify-center gap-2 transition-shadow ${isAuditing ? 'bg-stone-800 text-stone-400' : 'bg-gradient-to-r from-red-500 to-red-600 text-white shadow-[0_0_20px_rgba(255,77,77,0.3)] hover:shadow-[0_0_40px_rgba(255,77,77,0.5)]'}`}
              >
                {isAuditing && auditStatus === 'DISPATCHING RESOURCES...' ? (
